...
//
//
//  Created by Noah Martin on 1/26/21.
//
...
public class RBTreeNode<T: Comparable>: Equatable {
  ...
  public func lowerBound(of searchingKey: T) -> RBTreeNode<T>? {
    ...
    if key > searchingKey {
      return leftChild?.lowerBound(of: searchingKey)
    }

    if let rightLower = rightChild?.lowerBound(of: searchingKey), rightLower.key != nil {
      return rightLower
    }
    return self
  }

  public func upperBound(of searchingKey: T) -> RBTreeNode<T>? {
    guard let key = key else { return nil }

    if key == searchingKey {
      return rightChild?.upperBound(of: searchingKey)
    }
    ...
    if let leftUpper = leftChild?.upperBound(of: searchingKey), leftUpper.key != nil {
      return leftUpper
    }
    return self
  }

  public func traverseInOrder(process: (RBTreeNode<T>) -> Bool) {
    leftChild?.traverseInOrder(process: process)
    guard process(self) else { return }
...
extension RedBlackTree {
  ...
  private func property2() -> Bool {
    if root.color == .red {
      logger.info("Property-Error: Root is red")
      return false
    }
    return true
  }
    ...
    if let leftChild = node.leftChild, let rightChild = node.rightChild {
      if node.color == .red {
        if !leftChild.isNullLeaf && leftChild.color == .red {
          logger.info(
            "Property-Error: Red node with key \(String(describing: node.key)) has red left child"
          )
          return false
        }
        if !rightChild.isNullLeaf && rightChild.color == .red {
          logger.info(
            "Property-Error: Red node with key \(String(describing: node.key)) has red right child"
          )
          return false
        }
      }
  ...
  private func property5(node: RBNode) -> Int {
    ...
    if left == -1 || right == -1 {
      ...
      logger.info(
        "Property-Error: Black height violated at node with key \(String(describing: node.key))"
      )
      return -1
    }
  }
