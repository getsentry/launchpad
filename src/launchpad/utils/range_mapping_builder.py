...
class RangeMappingBuilder:
    ...
    def _map_load_commands(self, range_map: RangeMap) -> None:
        ...
        commands = self.parser.binary.commands
        if not commands:
            return
        ...
        for section in sections:
            ...
            try:
                if size == 0 or offset == 0:
                    logger.debug(f"Skipping section {name} with no file presence")
                    continue

                tag = self._categorize_section(name)

                range_map.add_range(offset, offset + size, tag, f"section_{name}")

            except Exception as e:
                logger.debug(f"Failed to map section {name}: {e}")
