...
class TreemapBuilder:
    ...
    def build_file_treemap(self, file_analysis: FileAnalysis) -> TreemapResults:
        ...
        """
        logger.info("Building file-based treemap")
    ...
    def _create_file_element(self, file_info: FileInfo, display_name: str) -> TreemapElement:
        ...
        return TreemapElement(
            ...
            element_type=self._get_file_type(file_info),
            path=file_info.path,
            details={
                "fileType": file_info.file_type,
                "hash": file_info.hash_md5,
            },
        )
    ...
    def _get_file_type(self, file_info: FileInfo) -> TreemapType:
        ...
        if (
            file_type == ""
            and "/" not in file_info.path
            and "." not in os.path.basename(file_info.path)
        ):
    ...
    def _calculate_category_breakdown(
        ...
        for file_type, files in file_analysis.files_by_type.items():
            for file_info in files:
                treemap_type = self._get_file_type(file_info)
                category = treemap_type.value

                breakdown[category]["install"] += file_info.size
                breakdown[category]["download"] += self._estimate_download_size(file_info)
