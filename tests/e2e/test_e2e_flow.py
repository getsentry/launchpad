...
def wait_for_processing(artifact_id: str, timeout: int = 180, check_interval: int = 3) -> Dict[str, Any]:
...
class TestE2EFlow:
    ...
    def setup_class(cls):
        ...
        print("=== Uploading and dispatching all tasks ===")

        if ANDROID_APK_FIXTURE.exists():
            upload_artifact_to_mock_api("test-android-apk-001", ANDROID_APK_FIXTURE)
            dispatch_task("test-android-apk-001", "test-org", "test-android-project")

        if IOS_FIXTURE.exists():
            upload_artifact_to_mock_api("test-ios-001", IOS_FIXTURE)
            dispatch_task("test-ios-001", "test-org", "test-ios-project")

        if ANDROID_AAB_FIXTURE.exists():
            upload_artifact_to_mock_api("test-android-aab-001", ANDROID_AAB_FIXTURE)
            dispatch_task("test-android-aab-001", "test-org", "test-android-project")

        dispatch_task("test-nonexistent-artifact", "test-org", "test-project")

        print("=== All tasks dispatched, processing in parallel ===\n")
    ...
    def test_android_apk_full_flow(self):
        ...
        results = wait_for_processing("test-android-apk-001")
        ...
        size_analysis = get_size_analysis_raw("test-android-apk-001")
    ...
    def test_ios_xcarchive_full_flow(self):
        ...
        results = wait_for_processing("test-ios-001")
        ...
        size_analysis = get_size_analysis_raw("test-ios-001")
    ...
    def test_android_aab_full_flow(self):
        ...
        results = wait_for_processing("test-android-aab-001")
        ...
        assert metadata["android_app_info"]["has_proguard_mapping"] is True

        assert results["has_size_analysis_file"]

        size_analysis = get_size_analysis_raw("test-android-aab-001")
